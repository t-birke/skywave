...
| File | Role |
|---|---|
| `pages/Home.tsx` | Top-level: chooses LIVE vs REPLAY, renders `<HoloGlobe>`, the HUD (status dot, stage, visitor count, **LIVE/6H/24H/7D/30D** preset buttons, replay progress bar), the **join-QR overlay**, the **call-in overlay**, and the inconspicuous seat-toggle dot; loads the option-image map. **Seeds the displayed stage from `Demo_Session__c.State__c`** (one `fetchActiveSession` read, shared with the seat toggle + the QR scope) so the status shows before the first live event; a relay `stage_changed` event overrides it. |
...
| `components/CallJoinOverlay.tsx` | The **call-in** companion to the QR — a phone-icon-only button just left of the QR card that **reveals the Skywave voice-agent number** (`+1 (615) 909-8098`) as a holographic pill on click. Self-contained (independent of the QR card's enlarge/hide state); shares the HUD cyan palette so the scan-to-join and call-to-join affordances read as a pair. |
