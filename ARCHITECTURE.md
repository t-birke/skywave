...
| `install.sh` | **Canonical tiered installer** (SDO target). Core / `--with-observability` / `--with-heroku` / `--all`; idempotent + `--resume`-able. Source of truth for every setup command; the skill conducts it. Supersedes the scratch-org `orgInit.sh`. |
